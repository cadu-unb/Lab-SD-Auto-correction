...
def gerar_questionario(lista_questoes: list, diretorio_questoes: Path = DIR_QUESTIONS) -> str:
    """
    Gera o código LaTeX para a seção de questionário a partir de uma lista
    de nomes de arquivos de questões.

    Args:
        lista_questoes (list): Uma lista de strings com os nomes dos arquivos (ex: ['A1E1.txt', 'A2E3.txt']).
        diretorio_questoes (Path, optional): O caminho para a pasta de questões. Default é DIR_QUESTIONS.

    Returns:
        str: A string contendo o código LaTeX para o questionário.
    """
    questionario_latex = ""
    ...
    # Itera sobre a lista de questões fornecida, em vez de todos os arquivos da pasta
    for i, nome_arquivo in enumerate(lista_questoes, 1):
        file_path = diretorio_questoes / nome_arquivo
        
        # Adiciona verificação para garantir que o arquivo de questão existe
        if not file_path.exists():
            print(f"Aviso: Arquivo de questão '{nome_arquivo}' não encontrado em {diretorio_questoes}. Pulando.")
            continue
